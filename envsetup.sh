...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^rising_") ; then
        RISING_BUILD=$(echo -n $1 | sed -e 's/^rising_//g')
    else
        RISING_BUILD=
    fi
    export RISING_BUILD
...
function lunch()
{
    ...
    if [ "$1" ]; then
        ...
        print_lunch_menu
        echo "Which would you like? [rising_arm-eng]"
        echo -n "Pick from common choices above (e.g. 13) or specify your own (e.g. rising_barbet-eng): "
        read answer
        used_lunch_menu=1
    fi
    ...
    then
        # if we can't find a product, try to grab it off the RisingOSS-devices GitHub
        T=$(gettop)
        cd $T > /dev/null
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $RISING_FIXUP_COMMON_OUT ]; then
